...
for cat in ['allwise','megeath','c2d','guth','iras','akari','akariyso','enoch06','enoch08','van_Kempen','2mass']:
...
#print '### STEP 8.5: DO IRAS20050 & NGC2071 IRAC PHOTOMETRY ###'
## scripts outputs newtable.data
#os.system("/cardini3/mrizzo/anaconda/bin/python IRAS20050_NGC2071.py")
...
#for i in range(len(newsourcelist)):
#	for col in columnlist:
#		if "F" not in col and newsourcelist["e_"+col][i]<0.1*newsourcelist[col][i]:
#			newsourcelist["e_"+col][i] = 0.1*newsourcelist[col][i] # imposes 10% minmum uncertainty everywhere

# adjusts error bars determination to determine the spectral index (otherwise the Spitzer and 2MASS fluxes constrain the fits too much)
for i in range(len(totsourcetable)):
	for col in columnlist:
		if "F" not in col and totsourcetable["e_"+col][i]<0.1*totsourcetable[col][i]:
			totsourcetable["e_"+col][i] = 0.1*totsourcetable[col][i] # imposes 10% minmum uncertainty everywhere
totsourcetable.more()

# spectral index is only determined from ~2-20um
totsourcetable_wrong_errors = p.calcAlpha(totsourcetable[metalist+columnlist+errorbars],columnlist[2:-19],errorbars[2:-19],newwavelengths[2:-19])
